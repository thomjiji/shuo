...
在“转录服务”中选择“豆包云端”，填写火山引擎语音控制台的 API Key，再点击“保存并测试”。默认使用流式语音识别 2.0 小时版；旧版 App ID、Access Token 和资源 ID 可在展开项中配置。录音时音频会上传，屏幕底部的单行字幕条显示实时文字，从右向左循环滚动，粘贴完成后收起。主窗口也可查看本次转录。需要联网并按服务用量计费，凭据保存在 Windows 凭据管理器中。选择“本地模型”并保存即可切回本地听写。
