...
- 底部居中的无焦点 indicator，只在录音和转写期间显示蓝色波形。
- 主窗口提供 PowerToys 风格的全局快捷键设置，默认 `Ctrl+Alt+\`，可在编辑对话框保存、重置或清除。
- 主窗口最小尺寸为 840 x 600，避免设置卡片和快捷键控件在缩小时重叠。
...
1. 启动应用。
2. 如需修改快捷键，点击“录音触发快捷键”右侧的铅笔图标，按下新组合键后点击“保存”。
3. 在任意普通权限应用中按一次已设置的快捷键（默认 `Ctrl+Alt+\`）开始录音。
4. 再按一次停止录音并开始转写。indicator 会显示蓝色波形。
5. 最终文本会自动粘贴到原来的前台输入框。
