...
`TranscriptHistory` 将最终文字、完成时间和服务名称追加到 `%LOCALAPPDATA%\Shuo\history.jsonl`，每次写入后刷新到磁盘。读取时跳过并报告损坏行；追加前补换行，避免上一次中断留下的残行吞掉新记录。历史页按写入顺序倒序显示，每次增加 50 条，不截断磁盘记录。保存失败会显示错误并继续粘贴；粘贴失败不删除已保存记录。`partial` 只更新浮窗，不进入历史。
