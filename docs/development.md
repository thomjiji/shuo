...
日常入口为“语音输入、实时字幕、文字朗读”，统一的“设置”页与“关于”放在导航底部。设置页按提供方组织连接、模型和凭据，显式保存；功能页只保存自己的使用偏好，不覆盖尚未保存的服务连接。Mac 共用地址映射到识别、字幕和朗读配置；已有不同主机或自定义端口保持分别配置。侧栏按窗口宽度依次显示完整导航、仅图标和折叠菜单。语音输入通过已有 worker 获取麦克风声音；全局快捷键将结果粘贴到光标处，页面试用按钮将结果留在试用区。听写、字幕和朗读快捷键分别保存启用状态；关闭听写快捷键只注销系统热键，保留组合键和页面试用。“去掉末尾句号”位于“语音输入 -> 输入设置”。实时字幕固定使用系统回环音频，原文字幕直接调用 Mac ASR，翻译字幕使用现有翻译会话；字幕快捷键与页面按钮使用相同的语言偏好，不受当前页面影响。`daily` 设置仅保存字幕语言，不保存字幕文字；保存时保留其他设置节点。听写、字幕、朗读和更新互斥执行。

登录自启动使用当前用户的 `HKCU\Software\Microsoft\Windows\CurrentVersion\Run`，不需要管理员权限。注册命令包含 `--startup`，使应用只在后台启动；固定值名保证重复开启不会产生多个入口。界面以注册表中的实际命令为准，应用确认旧命令属于 Shuo 后才把路径更新为当前可执行文件，关闭时也不删除无法确认归属的同名项。
...
主窗口使用 `NavigationView` 切换日常操作、设置、历史和关于页面。内容宽度按右侧视口减去两侧留白计算，最大为 920 DIP，并在右侧区域内居中。浮窗使用桌面亚克力背景，监听 `UISettings.ColorValuesChanged`，在 UI 线程同步更新浅色或深色材质、边框和文字颜色；不支持亚克力时使用纯色背景。浮窗保持不激活，通过背景配置保留亚克力效果；文字通过整体字形遮罩在左侧淡出，并平滑滚动显示最新内容。蓝点随音量缩放，重音触发扩散波纹，安静时缓慢呼吸；关闭系统动画时保持静态。关闭时释放材质控制器并解除监听。
...
音频识别、文字翻译和语音合成分别处理音频到文字、文字到文字、文字到音频。Windows 的 `SelfHostedAsrClient` 负责识别协议与停止后的尾段，`SelfHostedTextTranslator` 负责纯文字翻译；`SelfHostedTranslationSession` 只组合两者生成字幕，翻译失败会取消识别与采集。`TranslatedSpeechClient` 组合文字翻译与豆包语音，不创建识别会话。`ServiceSettings` 统一读取连接地址和凭据；各功能只读取所需配置，保留现有凭据键、显式设置的不同主机地址和旧配置兼容性。

Mac 的 `Translator` 和 `SpeechSynthesizer` 分别加载、预热并记录就绪状态；一个模型加载失败后释放该模型引用，继续加载另一个。`/health` 的 `capabilities.translation` 与 `capabilities.speech` 分别报告状态，顶层 `ready` 表示至少一个能力可用。文字翻译只要求翻译模型，原文朗读只要求语音模型，本地译读要求两者；Windows 按当前功能检查状态，兼容旧服务的顶层就绪字段。两个模型仍在同一进程内共用一个 GPU 执行线程和请求互斥锁，避免并行推理争用资源；服务启动时依次尝试预热两者后才开始接收请求。
