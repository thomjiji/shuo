...
主窗口使用 `NavigationView` 切换常规、转录服务和文本整理区域，切换页面时取消未保存的快捷键编辑。内容宽度按右侧视口减去两侧留白计算，最大为 920 DIP，并在右侧区域内居中。浮窗监听 `UISettings.ColorValuesChanged`，在 UI 线程同步更新主题、背景、文字和同色渐隐端点，关闭时解除监听。
