...
日常入口为“语音输入、实时字幕、文字朗读”，统一的“设置”页与“关于”放在导航底部。设置页按提供方组织连接、模型和凭据，显式保存；功能页只保存自己的使用偏好，不覆盖尚未保存的服务连接。Mac 共用地址映射到识别、字幕和朗读配置；已有不同主机或自定义端口保持分别配置。侧栏按窗口宽度依次显示完整导航、仅图标和折叠菜单。语音输入通过已有 worker 获取麦克风声音；全局快捷键将结果粘贴到光标处，页面试用按钮将结果留在试用区。“去掉末尾句号”位于“语音输入 -> 输入设置”。实时字幕固定使用系统回环音频，原文字幕直接调用 Mac ASR，翻译字幕使用现有翻译会话；字幕快捷键与页面按钮使用相同的语言偏好，不受当前页面影响。`daily` 设置仅保存字幕语言，不保存字幕文字；保存时保留其他设置节点。听写、字幕、朗读和更新互斥执行。
