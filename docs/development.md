...
日常入口为“听、说、译”，服务配置与“关于”放在导航底部。侧栏按窗口宽度依次显示完整导航、仅图标和折叠菜单。“去掉末尾句号”位于“听”的输入框与剪贴板模式；“关于”显示版本与更新。“听”的麦克风输入框与剪贴板模式使用已有 worker；字幕模式通过统一的音频采集选择麦克风或系统回环，原文字幕直接调用 ASR，翻译字幕使用现有翻译会话。“译”调用独立文字翻译客户端，只有完整结束后才启用复制与继续朗读。`daily` 设置仅保存来源、去向和语言，不保存原文或译文；保存保留其他设置节点。文字翻译的取消令牌与退出信号关联，并参与听写、字幕、朗读和更新的互斥。
