...
在“设置 -> 自托管 Mac”中填写主机名或 IP，选择“语音合成模型”，根据需要修改“朗读提示词”，点击“测试连接与可用能力”，再保存服务设置。朗读提示词最多 300 个字符，对两个模型和两种音色均生效；默认为空，此时不向模型发送提示词，填写后可用于控制朗读风格。旧设置默认继续使用 `mlx-community/Qwen3-TTS-12Hz-1.7B-CustomVoice-8bit`。然后在“文字朗读”页选择原文朗读或中文译读，在“朗读设置”中将声音服务设为自托管 Mac，并选择 Serena 或 Vivian 音色。声音服务下方会显示当前 TTS 模型，无需重启页面。已有服务升级后需要执行 `up` 安装当前版本，才能接受新版应用发送的模型、音色、朗读提示词参数和更大的段落；服务版本过旧时，较长的段落会被拒绝并提示文字过长。
...
译读和实时字幕共用 Qwen3 翻译模型，同一时间只处理一个请求；其他请求会收到忙碌提示。实时字幕通过 `/v1/translation` 请求纯文字翻译，不生成音频。译读按段翻译，每段完成后开始流式生成音频；一次请求最多接受 3000 个 UTF-8 字节，语音合成按整段一次性生成，段内语调连贯，段与段之间会有轻微差别。暂停保留当前播放位置；播放器缓冲区填满后，Mac 等待客户端确认再继续生成。停止或断开连接会取消后续生成并释放会话。
