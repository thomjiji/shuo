...
返回 `ready: true` 后，在 Shuo 的“服务设置 -> 自托管 Mac”中填写主机名或 IP，测试连接并保存，再到“语音输入 -> 输入设置”选择自托管 Mac。共用地址会用于语音输入、字幕和朗读；连接不同主机或使用自定义端口时开启“分别指定主机”。随后可按快捷键听写，浮窗显示实时预览，停止后粘贴最终文字。
