...
服务先检测语音，再约每秒识别当前段落并更新预览。停顿约 600 毫秒后确认该段，之后只识别新的段落，已确认文字不再重新计算。停止录音会补齐不足一包的尾音，并等待所有段落完成。

连续讲话没有停顿时，每段最多 20 秒，避免推理开销随整次听写增长。强制切分可能影响边界处的词语或标点；自然停顿有助于识别。单次听写最长 30 分钟，当前服务每次只处理一路录音。短于约 120 毫秒的零碎语音会被过滤。
