...
返回 `ready: true` 后，在 Shuo 的“设置 -> 自托管 Mac”中填写主机名或 IP，分别选择“语音识别模型”和“实时字幕识别模型”，测试连接并保存，再到“语音输入 -> 输入设置”选择自托管 Mac。两个功能可独立选择 Qwen3-ASR 1.7B 或 0.6B；旧的实时字幕设置默认继续使用 1.7B。共用地址会用于语音输入、字幕和朗读；连接不同主机或使用自定义端口时开启“分别指定主机”。随后可按快捷键听写，浮窗显示实时预览，停止后粘贴最终文字。

在“语音输入的识别模型”中选择 Qwen3-ASR 1.7B 或 0.6B，保存服务设置后在下一次听写时生效。两个选项都使用 Mac 上的 MLX 服务和相同的实时分段预览流程。0.6B 的实际速度与识别效果可用自己的录音比较；服务端未安装所选模型时会明确报错。
