...
1. 安装并打开“说”。
2. 在“设置 -> 火山引擎 / 豆包”中填写转录 API，选择识别模型并保存，再到“语音输入 -> 输入设置”选择火山引擎。新电脑会自动创建默认配置，不需要先下载本机模型或安装 Pi。
...
在“设置”中展开“本机模型”。在模型选择框右侧点击“下载”下载约 811 MB 的 Q8_0 模型，选择框下方显示下载进度，可随时取消。应用使用 Windows 系统代理，下载完成后校验文件大小和 SHA-256，失败可重试。
...
如需使用自行准备的 GGUF 文件，可将其放入配置旁的 `models` 目录后重新打开设置页，或者在配置中指定路径。已有旧版配置的电脑会继续使用 `%LOCALAPPDATA%\WindowsDictation\settings.json`。使用本地模型时，可编辑自动创建的 `%LOCALAPPDATA%\Shuo\settings.json`：
...
在火山引擎开通[流式语音识别](https://www.volcengine.com/product/asr)，从语音控制台获取 API Key。在“设置 -> 火山引擎 / 豆包”中填写转录 API，选择识别模型并保存，再到“语音输入 -> 输入设置”选择火山引擎。默认资源为流式识别 2.0 小时版，已有配置中的资源和凭据会保留。
