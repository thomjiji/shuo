...
public sealed partial class MainWindow
{
    ...
    private void UpdateInstallControls()
    {
        ...
        var idle = _textTranslationCancellation is null && _readingCancellation is null && _translationCancellation is null && !_exiting && !_closed && !_dictationActive && !_togglePending && !_modelChanging && _pendingPastes == 0;
    ...
    {
        if (_textTranslationCancellation is not null || _readingCancellation is not null || _translationCancellation is not null || _updateManager is null || _checkingUpdate || _installingUpdate || _exiting || _closed || _dictationActive || _togglePending || _modelChanging || _pendingPastes > 0) return;
