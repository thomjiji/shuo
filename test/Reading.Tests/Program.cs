...
var macDoubao = new ReadingOptions(TranslateToChinese: true, SpeechRate: 15);
macDoubao.Validate();
Check(JsonSerializer.Deserialize<ReadingOptions>(JsonSerializer.Serialize(macDoubao)) == macDoubao,
    "Mac translation with Doubao speech survives settings roundtrip");
Check(!JsonSerializer.Deserialize<ReadingOptions>("{\"TranslateToChinese\":true,\"TranslationSpeechRate\":2}")!.UseSelfHostedTranslation, "previous cloud translation settings select Mac translation with Doubao");
Check(JsonSerializer.Deserialize<ReadingOptions>("{\"UseSelfHostedTranslation\":true}")!.UseSelfHostedTranslation, "existing local speech selection is preserved");
await Fails<ArgumentException>(() => { (macDoubao with { Speaker = "" }).Validate(); return Task.CompletedTask; },
    "Mac translation with Doubao requires a valid speech voice");
await Fails<ArgumentException>(() => { (macDoubao with { SpeechRate = 101 }).Validate(); return Task.CompletedTask; },
    "Mac translation with Doubao validates synthesis rate");
